/**
 * public/js/os/packages/PackageErrors.js
 * Standardized error model for AdityyaOS packaging and package registry operations.
 */

export class PackageError extends Error {
  /**
   * @param {Object} options
   * @param {string} options.code - Error code (e.g. EPACKAGE_INVALID, EMANIFEST_INVALID, etc.)
   * @param {string} options.message - Human-readable error description
   * @param {string} [options.appId=null] - Application ID
   * @param {Error|null} [options.cause=null] - Underlying error cause
   */
  constructor({ code, message, appId = null, cause = null }) {
    super(message);
    this.name = 'PackageError';
    this.code = code || 'EPACKAGE_ERROR';
    this.appId = appId;
    this.cause = cause;
  }

  /**
   * Return a snapshot-safe structured JSON representation.
   * @returns {{ name: string, code: string, message: string, appId: string|null }}
   */
  toJSON() {
    return {
      name: this.name,
      code: this.code,
      message: this.message,
      appId: this.appId
    };
  }
}
